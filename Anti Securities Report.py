...
if ticker_input and all_data and all_data.get('hist_1y') is not None:
    if True:
            ...
            tab_overview, tab_radar, tab_insiders, tab_news, tab_crowd = st.tabs(["📊 概览 (Overview)", "🕸️ 五维雷达 (Metrics)", "🏛️ 机构与资金 (Insiders)", "📰 相关新闻 (News)", "📈 众包预测 (Crowdsourcing)"])
            ...
            with tab_crowd:
                st.markdown("---")
                st.markdown("## 📈 众包财务预测量化 Agent <span style='font-size:0.72rem; opacity:0.6;'>输入你的口语化感知，AI自动转为量化基本面预期</span>", unsafe_allow_html=True)
                
                user_prediction = st.text_area("输入你的商业感知 (例如：下个季度他们家 AI 芯片肯定卖爆，利润起码涨两成)", height=100)
                
                if st.button("🤖 提交 AI 智能量化预测", key="btn_crowd_predict"):
...
"""
                                response = client.chat.completions.create(
                                    ...
                                    messages=[
                                        ...
                                        {"role": "user", "content": f"当前标的: {ticker_input}\n用户输入: {user_prediction}"}
                                    ],
                                    temperature=0.1
                                )
                                ...
                                if json_match:
                                    ...
                                    st.success("✅ 智能预测已成功量化并录入众包数据库！")
                                    
                                    p_data = parsed_data.get("predictions", {})
                                    c_a, c_b, c_c = st.columns(3)
                                    c_a.metric("预测财季", parsed_data.get("fiscal_quarter", "N/A"))
                                    c_b.metric("预测营收", f"{p_data.get('revenue_estimate', 'N/A')} {p_data.get('revenue_unit', '')}")
                                    c_c.metric("预测毛利率", f"{p_data.get('gross_margin_estimate', 'N/A')}")
                                    
                                    st.info(f"**核心逻辑提炼**：{parsed_data.get('user_logic_summary', '')}")
                                    st.json(parsed_data)
                                else:
